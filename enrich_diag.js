...
function parseArgs() {
  ...
  const opts = {
    ...
    pcCrawlOnly: null,      // [DIAG] crawl only these console slugs (comma list)
    pcCrawlProbe: false,    // [DIAG] read target+scroll+log gate, skip record fetch
    skipPcPricing: false,   // [DIAG] skip Pass 3 pricing only; leave Pass 3b crawl on
  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case '--pc-crawl-only': opts.pcCrawlOnly = (args[++i]||'').split(',').map(s=>s.trim()).filter(Boolean); break; // [DIAG]
      case '--pc-crawl-probe': opts.pcCrawlProbe = true; break; // [DIAG] fast: no product-page fetch
      case '--skip-pc-pricing': opts.skipPcPricing = true; break; // [DIAG] Pass 3 off, Pass 3b (crawl) on
...
async function passPriceChartingCrawl(enriched, opts) {
  ...
  // [DIAG] print the full discovered slug list once (confirms a target console
  // is even in the crawl set — rules out "never crawled" as a cause).
  console.log(`  [PC3B-DIAG] discovered ${consoles.length} consoles: ${consoles.join(', ')}`);
  // [DIAG] optional slug filter so a diagnostic run can hit just the big sets.
  if (opts.pcCrawlOnly && opts.pcCrawlOnly.length) {
    const requested = opts.pcCrawlOnly;
    const missing = requested.filter(s => !consoles.includes(s));
    if (missing.length) console.log(`  [PC3B-DIAG] requested slugs NOT in discovered list: ${missing.join(', ')}`);
    consoles = consoles.filter(s => requested.includes(s));
    console.log(`  [PC3B-DIAG] filtering to ${consoles.length} set(s): ${consoles.join(', ')}`);
  }
  ...
  try {
    ...
    for (const [setIdx, slug] of consoles.entries()) {
      ...
      let setHeaderText = '';  // [DIAG] raw header text the target regex ran against
      try {
        ...
        const targetProbe = await page.evaluate(() => {
          ...
          // [DIAG] grab the header region so a target=0 result shows WHY the
          // regex missed. Prefer the neighbourhood of any "Funko"/"items"/"Prices"
          // heading; fall back to the top of the page text.
          let ctx = '';
          const anchor = text.search(/Prices for|Funko .*Figures|\bitems\b/i);
          if (anchor >= 0) ctx = text.slice(Math.max(0, anchor - 40), anchor + 160);
          else ctx = text.slice(0, 200);
          return { count, ctx: ctx.replace(/\s+/g, ' ').trim() };
        });
        const targetCount = targetProbe.count;
        setHeaderText = targetProbe.ctx;   // [DIAG]
        if (targetCount > 0) console.log(`    target: ${targetCount} figures`);
        setTarget = targetCount;

        // ── Row loading — deterministic scroll-to-target ─────────────────────
        console.log('    [DIAG] entering scroll block, target=' + targetCount);   // [DIAG breadcrumb]
        ...
        // The OLD scroll loop's real flaw was giving up after a fixed 12 stalls,
        // accepting a partial load as done when the next AJAX batch was merely
        // slow/rate-limited (this is what produced 900-of-1681 one run, 1681 the
        // next). Fix: keep scrolling as long as we are still SHORT of target, with
        // a generous no-progress budget, nudging the scroll position (up a little,
        // then back to bottom) to re-arm the lazy-load trigger on a stall.
        ...
        console.log('    [DIAG] initial rowCountNow ok, rows=' + rows);   // [DIAG breadcrumb]
        let best = rows;
        let noProgress = 0;
        const MAX_ITERS = 800;            // hard safety cap
        const NO_PROGRESS_LIMIT = 40;     // ~40 × (700+ ms) ≈ 30–40s with zero new
        ...
        console.log('  (page error) ' + (e && e.message ? e.message : '(no message)'));   // [DIAG]
        console.log('  (stack) ' + (e && e.stack ? String(e.stack).split('\n').slice(0,4).join(' | ') : '(no stack)'));  // [DIAG]
        pageLoadFailed = true;
        continue;
      }
      pages++;
      console.log(`  ${slug}: ${stubs.length} rows loaded`);
      if (!opts.pcCrawlProbe) {   // [DIAG] probe mode skips product-page fetch/add
      ...
      // [PC3B-DIAG] the line that decides Case A vs Case B. If target=0 the gate
      // went soft (regex miss → see rawHeader). If target>0 but rowsLoaded<target,
      // the scroll stalled short (lazy-load ceiling).
      console.log(`  [PC3B-DIAG] slug=${slug} target=${setTarget} rowsLoaded=${setRowsLoaded} reached=${reachedTarget} acceptable=${acceptable} pageLoadFailed=${pageLoadFailed} rawHeader=${JSON.stringify((setHeaderText||'').slice(0,200))}`);
...
async function main() {
  ...
  if (!opts.skipPc && !opts.skipPcPricing) {
    stats.pricecharting = await passPriceCharting(enriched, opts);
  } else {
    console.log(`\n── Pass 3: PriceCharting — SKIPPED (${opts.skipPc ? '--skip-pc' : '--skip-pc-pricing'})`);
  }
